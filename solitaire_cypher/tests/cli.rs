...
mod tests {
    ...
    #[test]
    fn test_no_args() {
        let mut cmd = Command::cargo_bin("solitaire_cypher").expect("crate binary should be where expected");
        cmd.assert()
            .failure()
    ...
    fn test_illegal_arg_combo() {
        let mut cmd = Command::cargo_bin("solitaire_cypher").expect("crate binary should be where expected");
        cmd.arg("--encrypt").arg("--decrypt").arg("--passphrase moosepoop")
    ...
    fn test_unparsable_passphrase() {
        let mut cmd = Command::cargo_bin("solitaire_cypher").expect("crate binary should be where expected");
        cmd
            .arg("--passphrase"). arg("cryp%^&omicon")
            .arg("--encrypt")
    ...
    fn test_encrypt_happypath() {
        let mut cmd = Command::cargo_bin("solitaire_cypher").expect("crate binary should be where expected");
        cmd
            .arg("--passphrase"). arg("cryptonomicon")
            .arg("--encrypt")
            .write_stdin("SOLITAIRE");
        cmd
            .assert()
